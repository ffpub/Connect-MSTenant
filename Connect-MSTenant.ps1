...
param (
	...
	[Parameter(ParameterSetName='ShowHelp',Mandatory=$false)][Switch]$ShowHelp
...
function CheckParamsAndEnvironment() { 
    param( 
        [Switch]$AzureAD,
        [Switch]$AzAccounts
    )
...
function CheckModulesInstalled {
	...
	if (-not (Get-Module $($SpecifiedService.ModuleName)) ) { 
		Log "$($SpecifiedService.ModuleName): Not Found. Installing & Importing..."
		install-module $($SpecifiedService.ModuleName) ; import-module $($SpecifiedService.ModuleName) 
	} 
	if (Get-Module $($SpecifiedService.ModuleName)) {
		return $true
	}
	else { return $false}
}
...
# Connect-MgGraph -TenantId "adit.onmicrosoft.com" -Scope "Sites.FullControl.All", "Directory.ReadWrite.All"
...
$Script:ServiceList = @(
	@{
		...
		GetConnectedTenantNameCommand = [scriptblock]::Create(" (Get-MsolPartnerInformation).PartnerCompanyName ")
		IsConnectedCommand =   [scriptblock]::Create(" Get-MsolCompanyInformation -ErrorAction SilentlyContinue " )
		IsTenantConnectedCommand =   [scriptblock]::Create(" (Get-MsolCompanyInformation).DisplayName " )
	...
	@{
		...
		GetConnectedTenantNameCommand = [scriptblock]::Create(" (Get-AzContext | select -ExpandProperty Name).Split(' ')[0] ")
	...
	@{
		...
		ConnectTenantCommand = [scriptblock]::Create(" Connect-AzureAD -TenantId `$PartnerTenantID " )
		GetConnectedTenantNameCommand = [scriptblock]::Create(" (Get-AzureADTenantDetail).DisplayName ")
	...
	@{
		...
		ConnectCommand =   [scriptblock]::Create(" Connect-MgGraph -TenantId `$CSPTenantId -Scope 'Directory.ReadWrite.All'")
		ConnectTenantCommand = [scriptblock]::Create(" Connect-MgGraph -TenantId `$PartnerTenantId -Scope 'Directory.ReadWrite.All' " )
		GetConnectedTenantNameCommand = [scriptblock]::Create(" ")
	...
	if (CheckTenantInfoCSV) {
		...
		$Services_Params | % { 
			if ($_ -eq "AzAccounts") {
				...
				if ($Find) { $Script:AzSubscriptionInfo = $Script:TenantInfoList | Where {$_.AzSubscriptionId -and $_.AzSubscriptionId -ne "" -and $_.IsCSP -ne $True -and $_.AzSubscriptionName -like "*$Find*" -or ($_.AzSubscriptionName).Replace(" ","") -like "*$Find*"}}
				else {
				...
				if (-not $Script:PartnerTenantInfo) {
					$Script:PartnerTenantInfo = $Script:TenantInfoList | where {$_.Name -like "*$Find*" -or $_.DefaultDomainName -like "*$Find*" -or ($_.Name).Replace(" ","") -like "*$Find*"} | Sort-Object Name -Unique

					if (($PartnerTenantInfo).Count -gt 1 ) { 
						$Script:PartnerTenantInfo = $($PartnerTenantInfo | sort Name ) | SelectFromSearch -PropertyNames "DefaultDomainName", "Name"
